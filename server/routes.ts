...
export async function registerRoutes(
  ...
  app.post(api.auth.login.path, async (req, res) => {
    try {
      ...
      if (!user) {
        user = await storage.createUser({ username, displayName: username, credits: 0, isSubscribed: false, roles: [] });
      }
  ...
  app.patch(api.auth.updateRoles.path, async (req, res) => {
    ...
    try {
      ...
      const finalRoles = hasMinistry ? [...selfServiceRoles, 'ministry'] : selfServiceRoles;
      const user = await storage.updateUserRoles(req.session.userId, finalRoles);
      res.status(200).json(user);
    } catch (err) {
      if (err instanceof z.ZodError) res.status(400).json({ message: err.errors[0].message });
      else res.status(500).json({ message: "Internal error" });
  ...
  app.get(api.projects.get.path, async (req, res) => {
    ...
    if (!isSubscribed && !isCreator) {
      ...
      project.submissions = project.submissions.filter(s => s.visibility === "public");
    }
  ...
  app.post(api.projects.create.path, async (req, res) => {
    ...
    if (!projectCreator?.isSubscribed) return res.status(403).json({ message: "Active membership required to create projects" });
    try {
      const input = api.projects.create.input.parse(req.body);
      const project = await storage.createProject({ ...input, creatorId: req.session.userId });
  ...
  app.post(api.files.create.path, async (req, res) => {
    ...
    if (!fileUser?.isSubscribed) return res.status(403).json({ message: "Active membership required to upload files" });
    try {
  ...
  app.post(api.investments.create.path, async (req, res) => {
    if (!req.session?.userId) return res.status(401).json({ message: "Not logged in" });
    const user = await storage.getUser(req.session.userId);
    if (!user?.isSubscribed) return res.status(403).json({ message: "Must be subscribed to invest" });
    try {
      ...
      if (existing.length >= 3) return res.status(403).json({ message: "Maximum of 3 investors reached" });
      if (input.percentage > 10) return res.status(403).json({ message: "Maximum 10% per investor" });
      if (existing.some(inv => inv.investorId === req.session.userId)) {
        return res.status(403).json({ message: "You have already invested in this project" });
      }
      ...
      const totalEquity = allInvestments.reduce((sum, i) => sum + i.percentage, 0);
  ...
  app.get(api.offerings.list.path, async (req, res) => {
    const result = await storage.getProjectOfferings(Number(req.params.projectId));
  ...
  app.post(api.coproducers.select.path, async (req, res) => {
    ...
    if (project.creatorId !== req.session.userId) {
      return res.status(403).json({ message: "Only the project creator can select co-producers" });
    }
    const projectOfferings = await storage.getProjectOfferings(projectId);
    if (projectOfferings.length === 0) {
      return res.status(400).json({ message: "No offerings yet — supporters must make offerings before co-producers can be selected" });
    }
  ...
  app.get(api.submissions.listAll.path, async (req, res) => {
    ...
    const visible = isSubscribed ? subs : subs.filter(s => s.visibility === "public");
  ...
  app.get(api.submissions.list.path, async (req, res) => {
    ...
    const viewer = req.session?.userId ? await storage.getUser(req.session.userId) : null;
    const isSubscribed = !!viewer?.isSubscribed;
    const subs = await storage.getProjectSubmissions(Number(req.params.projectId), types);
    const visible = isSubscribed ? subs : subs.filter(s => s.visibility === "public");
    res.status(200).json(visible);
  });
  ...
  app.post(api.submissions.create.path, async (req, res) => {
    ...
    try {
      ...
      const { ALL_SUBMISSION_TYPES, SUBMISSION_TYPES } = await import("@shared/schema");
      const submittingUser = await storage.getUser(req.session.userId);
      if (!submittingUser?.isSubscribed) {
        return res.status(403).json({ message: "Active membership required to submit contributions" });
      }
      if (!(ALL_SUBMISSION_TYPES as readonly string[]).includes(input.type)) {
        return res.status(400).json({ message: `Unknown submission type: ${input.type}` });
      }
      ...
      for (const role of userRoles) {
        const roleTypes = SUBMISSION_TYPES[role as keyof typeof SUBMISSION_TYPES];
        if (roleTypes) roleTypes.forEach(t => allowedTypes.add(t));
      }
      if (!allowedTypes.has(input.type)) {
        const reason = userRoles.length === 0
      ...
      const submission = await storage.createSubmission({
        ...input,
        licenseBestowalAmount: input.licenseBestowalAmount != null ? input.licenseBestowalAmount.toString() : undefined,
        sampleClearancePercent: input.sampleClearancePercent != null ? input.sampleClearancePercent.toString() : undefined,
        projectId: Number(req.params.projectId),
        userId: req.session.userId,
      });
  ...
  app.post('/api/donations', async (req, res) => {
    if (!req.session?.userId) return res.status(401).json({ message: "Not logged in" });
    try {
      const input = z.object({ amount: z.coerce.number().min(1) }).parse(req.body);
      ...
      const existingPass = await storage.getUserCypherPass(req.session.userId, year);
      if (!existingPass) {
        const yearDonations = await storage.getUserDonations(req.session.userId, year);
        const cumulative = yearDonations.reduce((s, d) => s + Number(d.amount), 0);
  ...
  app.get('/api/donations/me', async (req, res) => {
    if (!req.session?.userId) return res.status(401).json({ message: "Not logged in" });
    const year = new Date().getFullYear();
    const userDonations = await storage.getUserDonations(req.session.userId, year);
    const yearTotal = userDonations.reduce((s, d) => s + Number(d.amount), 0);
    const pass = await storage.getUserCypherPass(req.session.userId, year) ?? null;
    res.status(200).json({ donations: userDonations, yearTotal, hasPass: !!pass, pass });
  });
  ...
  app.post('/api/events/:eventId/votes', async (req, res) => {
    ...
    try {
      ...
      if (!voter?.isSubscribed) return res.status(403).json({ message: "Active membership required to vote" });
      ...
      if (!pass) return res.status(403).json({ message: "You need a Cypher Pass to vote" });
      // Max 4 votes per event
      const myVotes = await storage.getUserVotesForEvent(req.session.userId, eventId);
      if (myVotes.length >= 4) return res.status(403).json({ message: "You have already used all 4 votes for this event" });
      // Cannot vote for yourself
      if (input.artistUserId === req.session.userId) return res.status(403).json({ message: "You cannot vote for yourself" });
      ...
      const isEligible = ministryArtists.some(a => a.id === input.artistUserId);
      if (!isEligible) return res.status(400).json({ message: "That user is not an eligible ministry artist" });
      const vote = await storage.castVote({ userId: req.session.userId, eventId, artistUserId: input.artistUserId });
  ...
  app.get('/api/events/:eventId/my-votes', async (req, res) => {
    if (!req.session?.userId) return res.status(200).json({ count: 0, votes: [] });
    const eventId = Number(req.params.eventId);
    const userVotes = await storage.getUserVotesForEvent(req.session.userId, eventId);
  ...
  app.post('/api/projects/:id/negotiations', async (req, res) => {
    ...
    try {
      ...
      const user = await storage.getUser(req.session.userId);
      const userRoles = user?.roles ?? [];
      const hasNegotiableRole = userRoles.some(r => (NEGOTIABLE_ROLES as readonly string[]).includes(r));
      if (!hasNegotiableRole) {
        return res.status(403).json({ message: "Only Videographer/Marketing, Recording Engineer, or Dancer/Actor roles may submit negotiations" });
      }
  ...
  app.get('/api/projects/:id/negotiations', async (req, res) => {
    ...
    if (!project) return res.status(404).json({ message: "Project not found" });
    if (project.creatorId !== req.session.userId) {
      return res.status(403).json({ message: "Only the project creator can view negotiations" });
    }
  ...
  app.patch('/api/projects/:id/negotiations/:nId', async (req, res) => {
    ...
    if (!project) return res.status(404).json({ message: "Project not found" });
    if (project.creatorId !== req.session.userId) {
      return res.status(403).json({ message: "Only the project creator can respond to negotiations" });
    }
    try {
      const { status } = z.object({ status: z.enum(["accepted", "rejected"]) }).parse(req.body);
      const updated = await storage.updateNegotiationStatus(nId, projectId, status);
      if (!updated) return res.status(404).json({ message: "Negotiation not found for this project" });
  ...
  app.post('/api/submissions/:id/unlock-license', async (req, res) => {
    ...
    if (!sub.licenseBestowalAmount) return res.status(400).json({ message: "This submission has no license price set" });
    const alreadyUnlocked = await storage.hasUserUnlockedLicense(submissionId, req.session.userId);
    if (alreadyUnlocked) return res.status(400).json({ message: "You have already unlocked this license" });
    const unlock = await storage.unlockLicense(submissionId, req.session.userId, Number(sub.licenseBestowalAmount));
  ...
  app.get('/api/submissions/:id/unlocks/me', async (req, res) => {
    ...
    const submissionId = Number(req.params.id);
    const unlocked = await storage.hasUserUnlockedLicense(submissionId, req.session.userId);
  ...
  app.post('/api/stripe/webhook', async (req, res) => {
    ...
    if (!signature) return res.status(400).json({ error: 'Missing streturn res.status(401).json({ message: "Not logged in" });
    try {
      const user = await storage.getUser(req.session.userId);
      ...
      const circlePrice = priceSearch.data.find(p => {
        ...
        return typeof prod === 'object' && prod?.name === 'Producers Circle Pro';
      });
      if (!circlePrice) {
        return res.status(503).json({ message: "Subscription product not configured. Please run the seed script first." });
      }
  ...
  app.post('/api/stripe/portal', async (req, res) => {
    if (!req.session?.userId) return res.status(401).json({ message: "Not logged in" });
    const user = await storage.getUser(req.session.userId);
    if (!user?.stripeCustomerId) return res.status(400).json({ message: "No billing account found. Please subscribe first." });
    try {
      const stripe = await getUncachableStripeClient();
      const baseUrl = `${req.protocol}://${req.get('host')}`;
  ...
  app.post('/api/stripe/onboarding/complete', async (req, res) => {
    ...
    if (!user.isSubscribed) return res.status(403).json({ message: "Active subscription required" });
    try {
      const updated = await storage.updateUserOnboarding(req.session.userId, new Date());
  ...
  app.get('/api/stripe/subscription-status', async (req, res) => {
    ...
    try {
      const user = await storage.getUser(req.session.userId);
      if (!user?.stripeCustomerId) return res.status(200).json({ status: 'none' });
      const stripe = await getUncachableStripeClient();
      ...
      return res.status(200).json({ status: user?.isSubscribed ? 'active' : 'none' });
    }
  });
  ...
  app.post('/api/stripe/verify-session', async (req, res) => {
    ...
    try {
      ...
      if (checkoutSession.client_reference_id !== req.session.userId.toString()) {
      ...
      if (checkoutSession.payment_status === 'paid' || checkoutSession.status === 'complete') {
        ...
        const finalUser = await storage.updateUserOnboarding(req.session.userId, new Date());
  ...
  app.get('/api/users/:username', async (req, res) => {
    if (!req.session?.userId) return res.status(401).json({ message: "Not logged in" });
    const profile = await storage.getPublicProfile(req.params.username, req.session.userId);
  ...
  app.get('/api/users/:username/submissions', async (req, res) => {
    ...
    const { db } = await import("./db");
    const { eq } = await import("drizzle-orm");
    const rows = await db.select().from(subsTable).where(eq(subsTable.userId, user.id));
    const visible = isSubscribed ? rows : rows.filter((s: any) => s.visibility === "public");
    res.status(200).json(visible);
  });
  ...
  app.post('/api/users/:username/follow', async (req, res) => {
    ...
    if (target.id === req.session.userId) return res.status(400).json({ message: "Cannot follow yourself" });
  ...
  app.post('/api/messages', async (req, res) => {
    ...
    try {
      ...
      if (receiver.id === req.session.userId) return res.status(400).json({ message: "Cannot message yourself" });
      const msg = await storage.sendMessage(req.session.userId, receiver.id, input.body);
  ...
  app.patch(api.admin.grantMinistry.path, async (req, res) => {
    ...
    try {
      ...
      const updated = await storage.updateUserRoles(target.id, [...currentRoles, 'ministry']);
      res.status(200).json(updated);
    } catch (err) {
      if (err instanceof z.ZodError) res.status(400).json({ message: err.errors[0].message });
      else res.status(500).json({ message: "Internal error" });
  ...
  app.post('/api/playlists/:id/tracks', async (req, res) => {
    ...
    if (!submissionId || typeof submissionId !== 'number') return res.status(400).json({ message: "submissionId required" });
    try {
      const track = await storage.addTrackToPlaylist(playlistId, submissionId, req.session.userId);
  ...
  app.delete('/api/playlists/:id/tracks/:submissionId', async (req, res) => {
    ...
    try {
      await storage.removeTrackFromPlaylist(playlistId, submissionId, req.session.userId);
      res.json({ success: true });
    } catch (err: any) {
      res.status(403).json({ message: err.message });
    }
  ...
  app.patch('/api/playlists/:id/tracks/reorder', async (req, res) => {
    ...
    if (!Array.isArray(order) || !order.every(x => typeof x === 'number')) {
      return res.status(400).json({ message: "order must be array of submissionIds" });
    }
    try {
      await storage.reorderPlaylistTracks(playlistId, order, req.session.userId);
      res.json({ success: true });
    } catch (err: any) {
      res.status(403).json({ message: err.message });
    }
  ...
  app.get('/api/radio', async (req, res) => {
    ...
    const tracks = await storage.getRadioTracks(req.session.userId, user.isSubscribed ?? false);
    res.json(tracks);
  });
...
async function seedDatabase() {
  ...
  if (existingEvents.length === 0) {
    await storage.createEvent({
      ...
      description: "The inaugural annual gathering of the Free Soul Ecclesiastical Movement. Seven chosen co-producers perform live alongside ministry artists in a sovereign, members-only ceremony. Cypher Pass holders determine who takes the stage through their sacred 4-vote bestowal.",
  ...
  if (!user1) {
    user1 = await storage.createUser({ username: "alice", displayName: "Alice Artist", credits: 50, isSubscribed: true, roles: ["producer", "writer"] });
    await storage.updateUserOnboarding(user1.id, new Date());
  } else {
    const hasMinistry = (user1.roles ?? []).includes("ministry");
    const canonical = ["producer", "writer", ...(hasMinistry ? ["ministry"] : [])];
    user1 = await storage.updateUserRoles(user1.id, canonical);
    await storage.updateUserSubscription(user1.id, true);
    if (!user1.onboardingComplete) await storage.updateUserOnboarding(user1.id, new Date());
  }

  let user2 = await storage.getUserByUsername("bob");
  if (!user2) {
    user2 = await storage.createUser({ username: "bob", displayName: "Bob Producer", credits: 200, isSubscribed: true, roles: ["producer", "collaborator"] });
    await storage.updateUserOnboarding(user2.id, new Date());
  } else {
    const hasMinistry = (user2.roles ?? []).includes("ministry");
    const canonical = ["producer", "collaborator", ...(hasMinistry ? ["ministry"] : [])];
    user2 = await storage.updateUserRoles(user2.id, canonical);
    await storage.updateUserSubscription(user2.id, true);
    if (!user2.onboardingComplete) await storage.updateUserOnboarding(user2.id, new Date());
  }

  // ── Demo ministry artists so voting is testable out of the box ────────────
  let zara = await storage.getUserByUsername("zara");
  if (!zara) {
    zara = await storage.createUser({ username: "zara", displayName: "Zara Free Soul", credits: 0, isSubscribed: true, roles: ["ministry", "producer"] });
  } else if (!(zara.roles ?? []).includes("ministry")) {
    zara = await storage.updateUserRoles(zara.id, [...(zara.roles ?? []).filter(r => r !== "ministry"), "ministry"]);
  }

  let malik = await storage.getUserByUsername("malik");
  if (!malik) {
    malik = await storage.createUser({ username: "malik", displayName: "Malik Sovereign", credits: 0, isSubscribed: true, roles: ["ministry", "writer"] });
  } else if (!(malik.roles ?? []).includes("ministry")) {
    malik = await storage.updateUserRoles(malik.id, [...(malik.roles ?? []).filter(r => r !== "ministry"), "ministry"]);
  }

  const existingProjects = await storage.getProjects();

  let p1Id = existingProjects.find(p => p.title === "Neon Nights EP" && p.creatorId === user1.id)?.id ?? 0;
  let p2Id = existingProjects.find(p => p.title === "Acoustic Sessions" && p.creatorId === user2.id)?.id ?? 0;

  if (!p1Id) {
    const p1 = await storage.createProject({ title: "Neon Nights EP", description: "Synthwave exploration EP with 5 tracks. Looking for vocalists and additional synth layers.", creatorId: user1.id });
    p1Id = p1.id;
    await storage.initializeRoyaltySplits(p1Id);
    await storage.createFile({ projectId: p1Id, uploaderId: user1.id, name: "Synth Bass Stem", url: "https://storage.producerscircle.mock/synth.wav", type: "stem" });
    const seedInvestment = await storage.createInvestment({ projectId: p1Id, investorId: user2.id, amount: "50", percentage: 5 });
    await storage.upsertProducerSplit(p1Id, seedInvestment.percentage);
  }

  if (!p2Id) {
    const p2 = await storage.createProject({ title: "Acoustic Sessions", description: "Looking for vocalists and guitarists to collaborate on this organic, soulful project.", creatorId: user2.id });
    p2Id = p2.id;
  ...
  const seedSubs = [
    { projectId: p1Id, userId: user1.id, type: "beat" as const, title: "Neon Pulse Beat", description: "Main beat with 808s and synth chords at 128bpm" },
    { projectId: p1Id, userId: user2.id, type: "hook" as const, title: "City Lights Hook", description: "Melodic hook about driving through the city at night" },
    { projectId: p2Id, userId: user1.id, type: "mood-board" as const, title: "Earthy Tones Vision", description: "Visual direction: warm browns, candlelight, natural textures" },
    { projectId: p2Id, userId: user2.id, type: "collab-beat" as const, title: "Open Collab Groove", description: "Chill 90bpm groove — open for additional instruments" },
  ];

  for (const s of seedSubs) {
    const [existing] = await db.select().from(subsTable).where(
      and(eq(subsTable.title, s.title), eq(subsTable.userId, s.userId))
    );
    if (!existing) {
